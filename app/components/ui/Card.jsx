...
const Card = ({ name, work, image }) => {
    return (
        <div>
            <Image className="rounded-t-xl h-64" src={`/${image}.jpg`} width={500} height={500} alt="Persona1" />
            <div className='bg-primary rounded-b-xl p-5 flex flex-col gap-5'>
                <div className='flex justify-between items-center'>
                    <div>
                        <h3 className='text-secondary text-3xl font-bold'>{name}</h3>
                        <p className='text-tertiary'>{work}</p>
                    </div>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-7 text-tertiary">
